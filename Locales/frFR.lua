if TalentReminderLocaleCode ~= "fr" then
    return
end

local L = TalentReminderLocale

L["title"] = "Talent Reminder"
L["version"] = "Version"
L["subtitle"] = "Configure l'affichage du rappel lorsque vous entrez dans une instance sélectionnée."
L["messageLabel"] = "Texte du rappel"
L["fontSize"] = "Taille de police"
L["duration"] = "Durée totale"
L["fade"] = "Fondu"
L["sound"] = "Son"
L["readyCheckReminder"] = "Afficher le rappel lors d'un appel (Donjon ou raid uniquement)"
L["noSound"] = "Aucun son"
L["move"] = "Déplacer le rappel"
L["lock"] = "Verrouiller la position"
L["test"] = "Tester le rappel"
L["center"] = "Centrer la position"
L["moveHint"] = "Faites glisser pour déplacer"
L["info"] = "Commandes : /tr settings, /tr test, /tr move, /tr id, /tr instances"
L["moveOn"] = "mode déplacement activé."
L["moveOff"] = "mode déplacement désactivé."
L["instance"] = "Instance"

L["textColor"] = "Couleur du texte"
L["color1"] = "Blanc"
L["color2"] = "Rouge"
L["color3"] = "Vert"
L["color4"] = "Orange"

L["expansions"] = "Extensions"
L["allExpansions"] = "Toutes"
L["worldMenu"] = "Monde"
L["currentSeason"] = "Saison actuelle"
L["selectedExpansions"] = "%d sélectionnées"
